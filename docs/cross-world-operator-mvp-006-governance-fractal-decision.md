# MVP-006 Governance And Fractal Decision Prototype
...
- Prototype work order: [wo-urban-033-cross-world-governance-fde-prototype.yaml](subagents/work-orders/wo-urban-033-cross-world-governance-fde-prototype.yaml)

## 目的

`UE-XWORLD-MVP-006 Governance And Fractal Decision` は、Cross-world Operator Modeで出てきた評価結果、TODO、PR判断を、単一agentの独断ではなく、proposal、review、execution、oversightに分けて扱うtoy prototypeです。

このprototypeではdata contractを変更しません。MVP-005までに定義した評価結果を、FDE packet、三権分立風の役割分離、user oversight、three AI branches + meta-user構造へ接続するため、stateless API responseとUI panelだけを追加します。

## Versioning

このprototype追加は、Cross-world Operator Mode docs package の `0.7.0` MINOR更新です。
...
- Cross-world Operator Mode全体をskill meshとorchestration packにする前提を明記する。
...
## Skill / Orchestration Premise

Cross-world Operator Mode は、最終的に単一機能ではなく、再利用可能なskill集合と、それらを安全に呼び分けるorchestration群として扱います。

### Skill mesh

skillは、次のような責任単位に分けます。
...
- governance / FDE packet
- repo-as-skill / distributed ops
- intake lifecycle / public-safe validator

各skillは、source category、allowed I/O、human gate、rollback condition、failure stateを持ちます。

### Orchestration pack

orchestrationは、skill同士を直接つなぐだけでなく、次を束ねます。
...
- review handoff
- rollback condition
- stale / heartbeat / orphan tracking

recursive skill callは許容しますが、maximum depth、allowed I/O、loop guard、human approval boundaryが未定義なら実装しません。
...
- `GET /api/governance-fde`
  - proposal、review、execution、oversightの4層、FDEの5 step、decision候補、Numeric Operating Protocolのparking-lot状態、Future-Known Implementation Frame、Human Time-Sense Boundaryを返す。
- `POST /api/governance-fde/decide`
  - `decision` が `proceed` / `revise` / `watch` / `parking-lot` / `reject` のいずれかなら、レスポンス上のdecisionとして返す。
  - `proceed` は `tests`、`static-scan`、`review` 相当の3つ以上のevidenceを要求する。
  - `human_gate: false`、`user_as_agent: true` は `oversight_bypassed` で拒否する。
  - `future_claim: prophecy` は `future_claim_overreach` で拒否する。
  - `numeric_rule: implemented` は `numeric_rule_overreach` で拒否する。
  - `recursive_depth_unbounded: true` は `recursive_loop_unbounded` で拒否する。

## Prototype UI

右パネルの `Governance` でFDE decisionを選択し、user oversight、FDE steps、Numeric Operating Protocolのparking-lot理由を確認できます。

このUIは意思決定の実行エンジンではなく、decision packetとgateを可視化するreview surfaceです。外部API、cloud、Discord、GitHub issue、Linear issueへのwriteは行いません。
...
`XWORLD-TODO-026 Numeric Operating Protocol` は、数字仮説を採用しないためのguard実装として閉じます。Dispositionは `parking-lot` のまま維持します。
...
- 実装済みguard: `GET /api/governance-fde` は `numeric_protocol.status = parking-lot` を返し、`POST /api/governance-fde/decide` は `numeric_rule: implemented` を `numeric_rule_overreach` で拒否します。
...
- `GET /api/governance-fde` がgovernance layer、FDE steps、oversight boundaryを返す。
- `POST /api/governance-fde/decide` が定義済みdecisionだけを受け入れる。
- `proceed` はtests、scan、review相当のevidenceを要求する。
- human gate省略、user agent化、future overreach、numeric overreach、unbounded recursionを拒否する。
- UIからGovernance panelでdecision、oversight、FDE steps、numeric parking-lotを確認できる。
...
- Cross-world Operator Modeをskill meshとorchestration packにする前提が明記されている。
