...
このページは、Discord 導線を再開する時の参照用です。現時点の作業指示ではありません。

再開時に GitHub PR を Discord へ自動通知する場合は、Discord Webhook URL を GitHub Actions secret として保存します。
...
再開後に通知がうるさい、投稿先を間違えた、または secret を漏らした可能性がある場合:
