...
```bash
python tools/cloud_run_preflight.py --issue NEX-29 --format json
```

`fastapi` / `uvicorn` が入っていないローカル環境では、先に一時 venv を作ってから実行してください。
...
```bash
python tools/cloud_run_preflight.py --issue NEX-29 --format markdown \
...
```bash
python tools/cloud_run_preflight.py --issue NEX-29 --skip-smoke --format json
```
