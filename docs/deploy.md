...
この preflight は Docker も必須にしません。Cloud Run の `--source .` はローカル Docker ではなく Cloud Build 側でビルドできるため、Docker は local image build を手元で試したい場合だけ任意で使います。
...
| `scope.local_docker_required` | `false` |
| `checks[].name = local Docker is optional for this preflight` | `ok` |
