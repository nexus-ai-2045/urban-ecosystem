...
- 読み込む主要データは `pois.geojson`、`aois.geojson`、`roadnet.geojson`、`agent_profiles_N100.json`、`activity_plans.jsonl` (任意)、`poi_visit_records.jsonl`、`agent_states.jsonl`。
...
必須: `tick`, `day`, `time`, `agent_id`, `lat`, `lon`, `action`, `status`／任意: `current_poi_id`, `target_poi_id`, `route_mode` (`roadnet` または `linear_fallback`)。
...
| `activity_plans.jsonl` | JSONL | 入力 (任意) | schedule-driven destination selection 用の activity chain。無い場合は既存 rule-driven 挙動。 |
...
| `metrics.json` | JSON | 出力 | replay-derived metrics。地域データ・activity plan 差分比較に使う。 |
...
urban-ecosystem/
  ...
  tools/
    ...
    fetch_open_region_snapshot.py # open data fixture snapshot 正規化 (APIキーなし)
...
| `models.py` | POI、AOI、AgentProfile、ActivityPlan、AgentState、InteractionEventの型。 |
...
| `fetch_open_region_snapshot.py` | OSMnx / Overture 等から取得済みの fixture を検証し、POI/AOI/Road/Profile と `snapshot_manifest.json` を deterministic に出力する。first slice は live network を使わない。 |
...
python tools/urban_simulation_cli.py run \
  ...
  --activity-plans data/activity_plans.jsonl \
  --seed 42 \
  --out experiments/results/urban_demo
```

`--activity-plans` は optional input。指定が無い場合は既存の rule-driven simulation を使い、指定がある場合だけ `activity_plans.jsonl` の active activity を目的地選択へ優先適用する。同一スクリプトを Cloud Run Job のエントリポイントとしても使う。

### 12.2 Open regional fixture snapshot

```bash
python tools/fetch_open_region_snapshot.py \
  --fixture tests/fixtures/open_region/minimal \
  --out /tmp/urban_open_region_snapshot
```

WO-URBAN-014 first slice では live network / API キー / 課金 API を使わない。OSMnx / Overture 等から取得済みの小さな fixture を data-contract 形式として検証・正規化し、`snapshot_manifest.json` に source / license / retrieval mode / bbox / query / generated_at / tool version を記録する。既定の `generated_at` は fixture 用の deterministic timestamp で、同一 fixture 入力の byte 一致を保つ。

### 12.3 リプレイビュー起動 (ローカル開発)
...
| `activity_plans.jsonl` | `application/x-ndjson` | no | Optional activity chain plans。run dir に同梱された場合だけ返す。 |
...
| `metrics.json` | `application/json` | yes | JSON オブジェクト。地域データ差し替え時の到着率・移動 fallback・交流指標を比較する。 |

上記 11 ファイル以外のパスは 403 Forbidden を返す (パストラバーサル防止・許可リスト方式)。[事実: 設計決定/§21.3.1 — `ALLOWED_FILES = frozenset({...})` による先頭チェックを実装方針として確定]
