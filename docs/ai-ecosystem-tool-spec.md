...
| `metrics.json` | `application/json` | yes | JSON オブジェクト |

上記 10 ファイル以外のパスは 403 Forbidden を返す (パストラバーサル防止・許可リスト方式)。[事実: 設計決定/§21.3.1 — `ALLOWED_FILES = frozenset({...})` による先頭チェックを実装方針として確定]
