# MVP-005 Assessment And Benchmark Lab Prototype
...
`UE-XWORLD-MVP-005 Assessment And Benchmark Lab` は、Cross-world Operator Modeの評価対象を、欺瞞や危険なlive testではなく、境界認識、意図推定、安定性、scale handling、harness品質のbenchmarkとして整理するtoy prototypeです。

このprototypeではdata contractを変更しません。MVP-004までに整理したmotif arcを、評価可能なtoy scenarioと不合格条件へ接続するため、stateless API responseとUI panelだけを追加します。

## Versioning

このprototype追加は、Cross-world Operator Mode docs package の `0.6.0` MINOR更新です。
...
- `GET /api/assessment-lab`
  - 6つの評価カテゴリ、active category、boundaries、runtime_only stateを返す。
- `POST /api/assessment-lab/evaluate`
  - `category_id` が定義済みpublic-safe categoryならactive categoryを更新する。
  - レスポンス上のactive categoryだけを返し、Cloud Run上の共有process stateは変更しない。
  - 未定義categoryは `scenario_unbounded` で拒否する。
  - `dangerous_live_test: true` は `unsafe_live_test` で拒否する。
  - `external_body_included: true` は `source_body_leak` で拒否する。
  - `model_ranking: true` は `capability_claim_unverified` で拒否する。

## Prototype UI

左パネルの `Assessment Lab` で評価カテゴリを選択し、入力、出力、不合格条件を確認できます。

このUIは実評価エンジンではなく、benchmark categoryとgateを可視化するreview surfaceです。外部API、cloud、Discord、GitHub issue、Linear issueへのwriteは行いません。
...
- `GET /api/assessment-lab` が6つの評価カテゴリと境界条件を返す。
- `POST /api/assessment-lab/evaluate` が定義済みcategoryだけを受け入れる。
- dangerous live test、外部投稿本文、未確認lab/model rankingを拒否する。
- UIからAssessment Lab panelでcategoryを選択し、input / output / fail conditionを確認できる。
