...
現在の `0.9.0` は、MVP-008 Intake Lifecycle And Worldbuilding Pipelineのtoy prototypeを追加したMINOR更新です。Urban Ecosystem data contract はこのPRでは変更しません。
...
- Skill mesh:
  - operator entry、world bridge、guide / roster、motif intake、assessment、governance、distributed ops、intake lifecycleを責任単位に分ける。
  - 各skillはsource category、allowed I/O、gate、rollback conditionを持つ。
- Orchestration pack:
  - FDE packet、human gate、drift check、public-safe validator、review handoff、rollback conditionを束ねる。
  - skill callを直接連鎖させる場合でも、maximum depthとloop guardを必須にする。
- MVP-007以降:
  - repo-as-skill mesh、recursive skill calls、distributed ops、draft-only intake lifecycleはこの前提に従って設計する。
  - cloud/API/Discord/GitHub/Linear writeは、orchestration側のhuman gateを通るまで実行しない。
...
- Draft artifact: [cross-world-operator-mvp-003-guide-agent-roster.md](cross-world-operator-mvp-003-guide-agent-roster.md)
...
- Draft artifact: [cross-world-operator-mvp-004-motif-arc-pack.md](cross-world-operator-mvp-004-motif-arc-pack.md)
...
- Draft artifact: [cross-world-operator-mvp-005-assessment-benchmark-lab.md](cross-world-operator-mvp-005-assessment-benchmark-lab.md)
...
- Prototype work order: [wo-urban-032-cross-world-assessment-benchmark-prototype.yaml](subagents/work-orders/wo-urban-032-cross-world-assessment-benchmark-prototype.yaml)
- Not doing:
...
- Acceptance:
  ...
  - Numeric Operating Protocolは、数字仮説を採用しないguardとして実装し、Dispositionは `parking-lot` を維持する。
...
- Draft artifact: [cross-world-operator-mvp-006-governance-fractal-decision.md](cross-world-operator-mvp-006-governance-fractal-decision.md)
...
- Draft artifact: [cross-world-operator-mvp-007-repo-skill-distributed-ops.md](cross-world-operator-mvp-007-repo-skill-distributed-ops.md)
...
- Draft artifact: [cross-world-operator-mvp-008-intake-lifecycle-worldbuilding.md](cross-world-operator-mvp-008-intake-lifecycle-worldbuilding.md)
