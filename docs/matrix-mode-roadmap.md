...
| 判定 | Yes 条件 | 現状 |
|---|---|---|
| 残務ゼロ？ | TODO がすべて `完了`、または `保留` に human-readable reason と再開条件がある | Yes (2026-06-13 時点: TODO 表全 27 行を確認。M0-001〜M9-006 / M10-001〜M11-003 のすべてが `完了`。未着手・進行中・保留が 0 件。GSI 3D map layer は local deterministic adapter として実装済み。M6-001 は cybernetic_governance motif packet と data contract v0.7.5 が実装済み。M9-006 は duel_school motif packet と data contract v0.7.4 が実装済み。M11-003 は MATRIX panel optional fields 表示と E2E 25 件で確認済み。M9-005 は walled_society motif packet と data contract v0.7.3 が実装済み。M9-004 は unstable_city_core motif packet と data contract v0.7.2 が実装済み。M9-003 は oath_chain motif packet と data contract v0.7.1 が実装済み。M9-002 は PR #95 (cead42e) で exchange_pair motif packet と data contract v0.7.0 が実装済み。M11-001 は PR #94 / #95 の merge 前 out-of-band review comment が evidence として確認できる) |
| 実装完了してる？ | `sentinel_mvp`、`bridge_agent`、`guide_agent`、`operator_agent`、`sentinel_swarm` の contract / runtime / viewer / tests が揃い、M6-M10 の docs / UI / drift gate が完了している | Yes (2026-06-13 実測: full pytest 641 件 pass・fail 0、E2E Playwright `tests/e2e/test_ui.py` 25 件 pass、GSI 3D focused E2E 2 件 pass、drift gate は `matrix_mode_skill_check.py --check` と `docs_sync_check.py --check` の 2/2 が exit 0、`git diff --check` exit 0。body_network_boundary / command_review_channel と duel_style / duel_rank は takeover_start event に emit され、GSI 3D adapter は API key なし opt-in / fallback 降格を持つ) |
| 運用まで保証された？ | issue intake、worker packet、human gate、docs drift、E2E、公開境界、rollback/stop 条件が検証済み | Yes (2026-06-13 時点: issue intake = M0-003 template 確立済み / worker packet = parallel delegation map 文書化済み / human gate = PR #94 / #95 の merge 前 out-of-band review comment で evidence 確認 / docs drift = drift gate 2/2 exit 0 / E2E = Playwright 25 件 pass / 公開境界 = identity guard 実地通過 2 回 / rollback・stop 条件 = operator_agent human gate event、motif packet stop 条件、GSI 3D fallback 降格 E2E が docs / tests 化済み。全 7 条件に current evidence が揃った) |
...
| M11-003 | 完了 | MATRIX panel optional fields の E2E 表示保証を追加する | MP-002 `exchange_pair`、MP-003 `oath_chain`、MP-004 `unstable_city_core` の optional fields が viewer の MATRIX panel に event type ごとに表示され、E2E が固定 port 衝突なしで安定して通る | `tools/urban_viewer/ui_panels.js` / `tools/urban_viewer/styles.css` / `tests/e2e/test_ui.py::TestMatrixOptionalFieldsPanel` 3 件 |
