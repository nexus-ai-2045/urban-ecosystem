...
status: implemented (local deterministic adapter / external tile connection deferred)
owner: nexus_ai
created: 2026-06-12
updated: 2026-06-14
source: docs/matrix-mode-roadmap.md (Motif packet template) / public GSI 3D map references

## Influence summary

国土地理院「3次元地図可視化サイト」(2026-06-01 試験公開) に着想を得て、API キー不要の 3D 風都市表現を fallback viewer の第 3 の map adapter として採用する。初期実装では外部 tile fetch を行わず、既存 replay GeoJSON を deterministic な canvas 3D 表現へ投影する。リアルの写実再現ではなく、シミュレーターとしての立体都市表現に限定する。
...
- **deterministic 3D canvas 表現**: 初期実装は MapLibre / tile fetch に依存せず、既存 AOI / road / agent layer を 3D 風に描画する。MapLibre GL JS + 地理院ベクトルタイル接続は後続差し替え候補に留める。
- **建物風 extrusion**: 既存 AOI footprint を簡易 extrusion として描く。シミュレーター色を保つため、写実テクスチャは使わない。
...
- 地理院のサーバサイド機能 (住所検索ジオコーディング等) への依存。README が予告なき変更・終了を明記しているため、初期実装では外部 tile 読み込みも行わない。
...
| 要素 | 内容 | Evidence |
|---|---|---|
| place and environment | 渋谷 replay の AOI / road / agent を立体風に表現 | `gsi_3d_adapter.js` の表示 |
| rules of possibility | API key なし・外部 tile なし・既存 replay bounds の中だけで描画する | adapter の bounds 設定 |
...
- **試験公開リスク**: 3次元電子国土基本図・可視化サイトは試験公開。タイル URL / 仕様 / 提供範囲が予告なく変わり得る。初期実装は外部 tile へ接続せず、将来接続する場合も adapter 1 ファイルに依存を閉じ込める。
- **提供範囲未確認**: 建物 3D (高さ属性) の渋谷カバー範囲は本実装では依存しない。外部 tile 接続フェーズで再確認する。
...
- **公開境界**: この packet は local deterministic adapter として実装済み。GitHub issue / PR 作成や push は human gate (public PR review 経路 M11-001) を通す。

## Testable acceptance

- `docs/gsi-3d-map-layer.md` (本ファイル) が存在し、採用するもの / 採用しないもの / risk notes が分かれている。
- `tools/urban_viewer/gsi_3d_adapter.js` が `map_adapter.js` の interface を満たす。
...
- 採用: local deterministic canvas adapter / 出典表示 / 後続 MapLibre + 地理院ベクトルタイル差し替え可能性
- 非採用: サーバサイド機能依存 / 課金 API / 写実テクスチャ / 初期実装での外部 tile fetch
- acceptance: API キーなし CI 維持 / replay byte 一致 / fallback 降格 E2E

まず後続実装前確認: 渋谷の建物 3D (高さ属性) 提供範囲。
```
