...
- skillは、operator entry、world bridge、guide / roster、motif intake、assessment、governance、distributed ops、intake lifecycleなどの責任単位に分ける。
- orchestrationは、FDE packet、human gate、drift check、public-safe validator、review handoff、rollback conditionを束ねる。
- skill callはrecursiveに拡張できるが、maximum depth、allowed I/O、source category、human gate、loop guardを必ず持つ。
- MVP-007以降は、この前提をrepo-as-skill mesh、orchestration pack、draft-only intake lifecycleの設計へ接続する。
