...
テスト対象:
  ...
  - GET /docs/{file}: ロードマップ HTML / 画像 preview の静的配信
...
class TestDocsStatic:
    def test_cross_world_roadmap_html_is_served(self, client_no_key):
        """Cloud Run 上でも roadmap HTML preview を閲覧できる。"""
        res = client_no_key.get("/docs/cross-world-operator-roadmap.html")

        assert res.status_code == 200
        assert "text/html" in res.headers["content-type"]
        assert "AIエージェントを、世界ごと動かせるようにする。" in res.text
        assert "assets/cross-world-roadmap-hero.png" in res.text

    def test_cross_world_roadmap_hero_image_is_served(self, client_no_key):
        """roadmap HTML が参照する hero 画像も同じ /docs 配下で配信する。"""
        res = client_no_key.get("/docs/assets/cross-world-roadmap-hero.png")
