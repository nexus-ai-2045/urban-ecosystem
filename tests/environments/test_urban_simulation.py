...
def test_oath_chain_fields_absent_when_matrix_off():
    """matrix_mode=False の run では matrix_events.jsonl を出力せず、
    agent_states は既存出力と byte 一致する (off-by-default 不変性)。

    MP-003 Testable acceptance (off-by-default) / contract v0.7.1。
    """
    pois, profiles = _make_simple_sim_inputs()
    seed = 42
    ticks = 4
    ...
    # matrix_events は空 (oath_chain フィールドも当然含まれない)
    assert sim1.matrix_events == [], "matrix_mode=False では matrix_events が空であるべき"
    assert sim2.matrix_events == [], "matrix_mode=False では matrix_events が空であるべき"
...
def test_oath_chain_fields_present_in_takeover_start():
    """matrix_mode=True の run で takeover_start event に hierarchy_rank と sworn_duty が含まれる。
    同一 seed 2 回 run で値が一致する (決定論)。

    MP-003 Testable acceptance (takeover_start fields) / contract v0.7.1。
    """
    pois, profiles = _make_simple_sim_inputs()
    seed = 77
    ...
    def _run():
        sim = Simulation(
            pois, profiles,
            seed=seed, ticks=4, run_id="oath_test",
            matrix_mode=True,
            matrix_agent_id=0,
            matrix_ttl_ticks=3,
            matrix_oath_chain_rank=1,
            matrix_sworn_duty="threat_containment",
        )
        sim.simulate()
        return sim
    ...
    # oath_chain フィールドの存在確認
    assert "hierarchy_rank" in evt, "hierarchy_rank が takeover_start に含まれるべき"
    assert "sworn_duty" in evt, "sworn_duty が takeover_start に含まれるべき"

    # 値の確認
    assert evt["hierarchy_rank"] == 1, "hierarchy_rank は指定値と一致するべき"
    assert evt["sworn_duty"] == "threat_containment", "sworn_duty は指定値と一致するべき"
...
# ─────────────────────────────────────────────────────────────────────────────
# MP-004: unstable_city_core motif packet (v0.7.2)
# ─────────────────────────────────────────────────────────────────────────────

def test_unstable_city_core_fields_absent_when_matrix_off():
    """matrix_mode=False の run では matrix_events.jsonl を出力せず、
    agent_states は既存出力と byte 一致する (off-by-default 不変性)。

    MP-004 Testable acceptance (off-by-default) / contract v0.7.2。
    """
    pois, profiles = _make_simple_sim_inputs()
    seed = 42
    ticks = 6
    ...
    # matrix_events は空 (unstable_city_core フィールドも当然含まれない)
    assert sim1.matrix_events == [], "matrix_mode=False では matrix_events が空であるべき"
    assert sim2.matrix_events == [], "matrix_mode=False では matrix_events が空であるべき"
...
def test_unstable_city_core_fields_present_in_stale_report():
    """matrix_mode=True かつ matrix_swarm_stale_tick 指定 run で stale_report event に
    core_instability_level と stabilization_phase が含まれる。
    同一 seed 2 回 run で値が一致する (決定論)。

    MP-004 Testable acceptance (stale_report fields) / contract v0.7.2。
    """
    pois, profiles = _make_simple_sim_inputs()
    seed = 55
    ...
    def _run():
        sim = Simulation(
            pois, profiles,
            seed=seed, ticks=8, run_id="unstable_core_test",
            matrix_mode=True,
            matrix_agent_id=0,
            matrix_ttl_ticks=6,
            matrix_swarm_heartbeat_tick=1,
            matrix_swarm_stale_tick=5,
            matrix_swarm_stale_after_ticks=3,
            matrix_core_instability_level=7,
            matrix_stabilization_phase="collapse",
        )
        sim.simulate()
        return sim
    ...
    # stale_report event を抽出
    stale_a = [e for e in sim_a.matrix_events if e["type"] == "stale_report"]
    stale_b = [e for e in sim_b.matrix_events if e["type"] == "stale_report"]

    assert len(stale_a) == 1, "stale_report が 1 件出力されるべき"
    evt = stale_a[0]

    # unstable_city_core フィールドの存在確認
    assert "core_instability_level" in evt, "core_instability_level が stale_report に含まれるべき"
    assert "stabilization_phase" in evt, "stabilization_phase が stale_report に含まれるべき"

    # 値の確認
    assert evt["core_instability_level"] == 7, "core_instability_level は指定値と一致するべき"
    assert evt["stabilization_phase"] == "collapse", "stabilization_phase は指定値と一致するべき"

    # 決定論: 同一 seed 2 回の結果が一致
    assert stale_a == stale_b, "同一 seed で stale_report の内容が一致するべき"
...
# ─────────────────────────────────────────────────────────────────────────────
# MP-007: mirror_episode motif packet (v0.7.6)
# ─────────────────────────────────────────────────────────────────────────────

def test_mirror_episode_fields_absent_when_matrix_off():
    """matrix_mode=False の run では matrix_events.jsonl を出力せず、
    agent_states は既存出力と byte 一致する (off-by-default 不変性)。

    MP-007 Testable acceptance (off-by-default) / contract v0.7.6。
    """
    pois, profiles = _make_simple_sim_inputs()
    seed = 42
    ticks = 4
    ...
    # matrix_events は空 (mirror_episode フィールドも当然含まれない)
    assert sim1.matrix_events == [], "matrix_mode=False では matrix_events が空であるべき"
    assert sim2.matrix_events == [], "matrix_mode=False では matrix_events が空であるべき"
...
def test_mirror_episode_fields_present_in_takeover_start():
    """matrix_mode=True の run で takeover_start event に identity_ambiguity_level と
    deception_risk が含まれる。同一 seed 2 回 run で値が一致する (決定論)。

    MP-007 Testable acceptance (takeover_start fields) / contract v0.7.6。
    """
    pois, profiles = _make_simple_sim_inputs()
    seed = 55
    ...
    def _run():
        sim = Simulation(
            pois, profiles,
            seed=seed, ticks=6, run_id="mirror_episode_test",
            matrix_mode=True,
            matrix_agent_id=0,
            matrix_ttl_ticks=5,
            matrix_identity_ambiguity_level=2,
            matrix_deception_risk="monitored",
        )
        sim.simulate()
        return sim
    ...
    # mirror_episode フィールドの存在確認
    assert "identity_ambiguity_level" in evt, "identity_ambiguity_level が takeover_start に含まれるべき"
    assert "deception_risk" in evt, "deception_risk が takeover_start に含まれるべき"

    # 値の確認
    assert evt["identity_ambiguity_level"] == 2, "identity_ambiguity_level は指定値と一致するべき"
    assert evt["deception_risk"] == "monitored", "deception_risk は指定値と一致するべき"
