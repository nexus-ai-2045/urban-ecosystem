...
PHASE_DESCRIPTIONS = {
    ...
    "Governance / Operations": "合議制、外部監視、スキル集合、分散運用、クラウド境界、世界構築の流れを運用へ接続する。",
}

PUBLIC_LABELS = {
    "Sentinel MVP": "エージェント視点に入る最小モード",
    "World Bridge Actor": "三つの世界を行き来する状態モデル",
    "Guide / Partner": "案内役と伴走役",
    "Agent Roster": "監視役と介入役の一覧",
    "Cyber-ops Milestones": "現場支援と統括ロール",
    "Equivalent Exchange Pair": "代償と修復のペア構造",
    "Pillar Council Arc": "守護者と評議会の構造",
    "Unstable Power Arc": "暴走する力と都市規模の不安定さ",
    "Boundary War Arc": "壁・境界・外界圧力",
    "Fighter Archetype Set": "修行・ライバル・型の対戦構造",
    "Social-Tech Mirror Lab": "技術が人間行動を歪める実験枠",
    "Judgment Game Arc": "正義・監視・推理の対抗構造",
    "Ecological Mediation Arc": "自然・群知能・環境交渉",
    "Pilot Sync Arc": "操縦者と同期圧力",
    "Human/AI Assessment Lab": "人間とAIの境界評価",
    "Three-world Layer": "三層世界",
    "Event + Music Layer": "イベント感と8bit音楽",
    "Next Motif Expansion Slot": "次の採用枠",
    "Post-Singularity Scenario Boundary": "シンギュラリティ後の扱い方",
    "Chaotic Three-Body World Benchmark": "混沌世界の評価軸",
    "Frontier AI Capability Layer Benchmark": "先端AI能力の評価軸",
    "Scale-Simplification Simulation Benchmark": "規模を広げると単純化する仮説",
    "Future-Known Implementation Frame": "未来像と現実実装の差分",
    "Human Time-Sense Boundary Benchmark": "人間の待ち時間感覚",
    "Numeric Operating Protocol": "数字による運用仮説",
    "FDE Packet Router Benchmark": "合議判断パケット",
    "Agent Harness Layer Benchmark": "モデル能力と運用器の切り分け",
    "User Oversight Fourth-Power Layer": "ユーザーによる外部監視",
    "Deliberative Separation-of-Powers Layer": "三権分立型の合議",
    "Repository-as-Skill Mesh": "リポジトリをスキル集合にする",
    "P2P Distributed Operations Spike": "分散運用の検討",
    "Cloud Capacity Envelope": "クラウド利用の上限設計",
    "Worldbuilding Extraction Pipeline": "世界構築を抽出する流れ",
    "place and environment": "場所と環境",
    "rules of possibility": "何が可能かのルール",
    "social fabric": "社会のつながり",
    "resources and power": "資源と権力",
    "history and memory": "歴史と記憶",
    "daily life signal": "日常の手触り",
    "change pressure": "変化を迫る圧力",
}
...
def _public_label(value: str) -> str:
    clean = value.strip(" `")
    return PUBLIC_LABELS.get(clean, clean)
...
def _phase_html(phases: list[Phase]) -> str:
    cards: list[str] = []
    for phase in phases:
        bullets = "\n".join(f"              <li>{html.escape(_public_label(item))}</li>" for item in phase.bullets[:12])
        cards.append(
            f"""          <article class="phase">
            <div class="phase-number">{phase.number:02d}</div>
            <div>
              <h3>{html.escape(phase.display_title)}</h3>
              <p>{html.escape(PHASE_DESCRIPTIONS.get(phase.source_title, phase.description))}</p>
              <span class="id">第{phase.number}段階</span>
            </div>
            <ul>
{bullets}
            </ul>
...
def generate_html(project_root: Path = PROJECT_ROOT) -> str:
    ...
    packet_cards = "\n".join(
        f"""          <article class="mini-card">
            <strong>{html.escape(_public_label(item))}</strong>
          </article>"""
  ...
  <div class="site-shell">
    ...
    <main>
      ...
      <section id="worlds">
        <h2>舞台は三つ</h2>
        <div class="journey">
          <article class="world physical"><small>1 / 現実</small><h3>人間が責任を持つ場所</h3><p>レビュー、承認、公開判断、費用、アカウント、クラウド利用などはここで扱う。</p><span class="id">現実レイヤー</span></article>
          <article class="world simulated"><small>2 / 仮想</small><h3>試しに動かす場所</h3><p>エージェント、世界ルール、ベンチマーク、シナリオを小さく試す。</p><span class="id">仮想レイヤー</span></article>
          <article class="world liminal"><small>3 / 境界</small><h3>意味をつなぐ場所</h3><p>現実と仮想の差分、違和感、保留、判断待ちを置く。</p><span class="id">境界レイヤー</span></article>
        </div>
      </section>
...
{phase_cards}
      ...
      <section id="gates">
        ...
        <div class="proof-grid">
          ...
          <article class="gate"><strong>外部書き込みは人間が止める</strong><p>外部チケット、内部管理ツール、クラウド、外部チャット、公開レビューは、人間レビュー前に自動実行しない。</p></article>
          <article class="gate"><strong>残った論点も行き先を持つ</strong><p>採用、保留、監視、対象外のどれかに分類し、採用なら必ず作業IDを持たせる。</p></article>
        </div>
      </section>
...
{non_goal_items}
      ...
      <section id="docs">
        <h2>正本と関連ドキュメント</h2>
        <div class="docs-row">
          <a class="doc-link" href="cross-world-operator-roadmap.md">ロードマップ正本<span>このページの生成元。</span></a>
          <a class="doc-link" href="cross-world-operator-todo.html">未実装TODO<span>残っている作業と保証マトリクス。</span></a>
          <a class="doc-link" href="cross-world-operator-linear-drafts.md">Linear draft<span>MVP単位に分けた内部管理用ドラフト。</span></a>
          <a class="doc-link" href="subagents/work-orders/wo-urban-018-cross-world-operator-roadmap.yaml">Work order<span>書き込み範囲、gate、受け入れ条件。</span></a>
        </div>
      </section>
    </main>

    <footer>
      <p>Generated from docs/cross-world-operator-roadmap.md / Status: draft / Owner: manager / Urban Ecosystem data contract は変更しません。</p>
    </footer>
  </div>
</body>
</html>
