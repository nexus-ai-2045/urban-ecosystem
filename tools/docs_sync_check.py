...
def _cross_world_version(path: Path) -> str:
    text = _read(path)
    if path.suffix == ".html":
        match = re.search(r"(?:Version|バージョン):\s*([0-9]+\.[0-9]+\.[0-9]+)", text)
    else:
...
def cross_world_drift_errors(project_root: Path = PROJECT_ROOT) -> list[str]:
    ...
    scan_paths = CROSS_WORLD_CORE_DOCS + [
        ...
        project_root / "docs" / "subagents" / "work-orders" / "wo-urban-032-cross-world-assessment-benchmark-prototype.yaml",
